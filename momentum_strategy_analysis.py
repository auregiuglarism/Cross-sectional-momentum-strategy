...
def run_momentum_strategy_analysis():
    ...
    return {
        # Index
        "momentum_df": momentum_df, # momentum scores
        "rank_df": rank_df, # ranks of indices based on momentum scores
        "returns_df": returns_df, # aligned indices returns for portfolio construction
        # Portfolios
        "portfolio_returns": portfolio_returns, # 5 portfolios returns
        "performance_summary": performance_summary, # performance metrics for portfolios
        # Index
        "annualized_return_assets": annualized_return_assets, # total return of indices
        "annualized_vol_assets": annualized_vol_assets, #standard deviation (volatility) of indices
        "sharpe_ratio_assets": sharpe_ratio_assets
    }

# --- Verification ---
# Function to check calculated values against reference values
def check_values(momentum_df, rank_df, portfolio_returns, annualized_return_assets, annualized_vol_assets, reference):
    """
...
if __name__ == "__main__":
    # Reference values given (DO NOT CHANGE)
    reference_march2023 = {
